import { GuildBasedChannel, Snowflake, TextChannel, VoiceChannel } from "discord.js";
...
export default class Audio {
  ...
  public static init() {
    ...
    this.player.on("trackAdd", (queue, track) => {
      (queue.metadata as TextChannel).send(`🎶 | Track **${track.title}** queued!`);
    });

    this.player.on("botDisconnect", (queue) => {
      (queue.metadata as TextChannel).send("❌ | I was manually disconnected from the voice channel, clearing queue!");
    });

    this.player.on("channelEmpty", (queue) => {
      (queue.metadata as TextChannel).send("❌ | Nobody is in the voice channel, leaving...");
    });
