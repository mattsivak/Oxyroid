...
const Whatsapp = new WhatsAppAPI(
	'EAAdVrWNI3u8BALb6ljN3fEXYfdhumE7nwYxvTeTm7KvMJZB5g3e6uvEsX5BJSdZBVmPaKs8WxEPI3lkpBoxAmLB2R0Chdw8EFzLAkR7u78MdZCsTpHC1XmAwWB7teNQgJ1Rqu9i26tCWSLA7l0GgeWB6H8JHR1zOazZA4ZBSfoSJOzR20Qg8LvHEECkEYQ1YQDh6ZCZA07uGwZDZD'
)
export default class Logger {
	...
	static log(
		...
		if (loggerTypes.includes('file')) {
			this.logToFile(
				...
				type,
				from,
				loggerType.includes('withoutFormating')
			)
		}
	...
	static logToDiscord(message: string, type: Type, from: string): void {
		...
		if (!this.firstLogTimeToDiscord) {
			this.firstLogTimeToDiscord = new Date()
				.toLocaleString()
				.replace(/\//g, '-')
				.replace(', ', '_')
				.replace(/\:/g, '-')
