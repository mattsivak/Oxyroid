...
export default new Command({
  ...
  async run(client, interaction) {
    ...
    if (!["high","medium","low","off"].includes(mode as string)) {
      await interaction.followUp({ content: `🚫 | wrong option. You can pick from: high, medium, low and off` });
      return false
    }
    ...
    await interaction.followUp({ content: `${icon} | Bass boost switched to **${mode}**` });
    return false
  },
})